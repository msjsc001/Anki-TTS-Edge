...
- **用户数据统一管理 & 主题重新设计 (v2.5)**：
  - **数据集中化**：所有用户文件迁移至 `%APPDATA%/Anki-TTS-Edge/`，目录英文命名（`audio/`、`logs/`），首次启动自动迁移旧数据。
  - **主题重新设计**：浅色模式采用暖色 Teal + Stone 配色，深色模式采用高对比 Teal-400 + Stone-900，视觉更舒适。
- **即点即读精确定位 (v2.4)**：点击文段中任意单词/字符，直接从该位置开始播放，定位更精确。
- **代码质量修复 & edge-tts 适配 (v2.3)**：
  - **修复 TTS 生成全面失效**：升级 `edge-tts` 至 7.2.7，修复微软 API 认证令牌过期导致的 403 错误。
  - **修复单词高亮跟随失效**：适配 API 变更，恢复单词级时间戳和实时高亮。
  - **窗口缩放防抖**、**日志轮转**、**消除裸异常**等多项代码质量加固。
- **深度清理 (v2.2)**：
...
- **全新的朗读体验 (v2.1)**：
  ...
  - **即点即读**：朗读模式下，点击文段中任意单词/字符，立即从该位置开始播放。
